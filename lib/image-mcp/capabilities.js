...
async function refreshCapabilities(credentials, cached, options) {
  ...
  if (status === 404 || status === 405) {
    const legacy = await legacyFallback(credentials, { ...options, fetch: fetchImpl });
    if (legacy) return { capability: legacy, etag: '' };
...
function normalizeActions(raw) {
  ...
  for (const [action, contract] of Object.entries(raw)) {
    if (!['generate', 'edit'].includes(action) || !isObject(contract) || contract.eligible !== true ||
      contract.endpoint !== (action === 'generate' ? '/v1/images/generations' : '/v1/images/edits')) {
...
function oneOfSchema(capabilities) {
  ...
  for (const model of capabilities.models) {
    for (const [action, actionContract] of Object.entries(model.actions)) {
      const properties = {
        model: { type: 'string', const: model.id, description: '来自当前 Key 能力合同的模型 ID。' },
        action: { type: 'string', const: action, description: '图片动作。' }
      };
      const merged = mergeProfileParametersForSchema(actionContract.profiles);
      for (const parameter of merged.parameters) {
        properties[parameter.name] = parameter.schema;
      }
      branches.push({
        type: 'object', properties,
        required: ['model', 'action', ...merged.required],
...
function mergeProfileParametersForSchema(profiles) {
  const byName = new Map();
  for (const profile of profiles) {
    for (const parameter of profile.parameters) {
      if (!byName.has(parameter.name)) byName.set(parameter.name, []);
      byName.get(parameter.name).push(parameter);
    }
  }
  const parameters = [];
  const required = [];
  for (const [name, variants] of byName.entries()) {
    ...
    for (const variant of variants) {
      const schema = parameterJSONSchema(variant);
      if (!uniqueSchemas.some((candidate) => JSON.stringify(candidate) === JSON.stringify(schema))) uniqueSchemas.push(schema);
    }
    parameters.push({ name, schema: uniqueSchemas.length === 1 ? uniqueSchemas[0] : { anyOf: uniqueSchemas } });
    if (variants.length === profiles.length && variants.every((parameter) => parameter.required)) required.push(name);
  }
...
export function summarizeImageCapabilities(capabilities, options = {}) {
  return {
    ...
    legacy: capabilities.legacy === true,
...
async function legacyFallback(credentials, options) {
  if (options.allowLegacyFallback !== true || !sameOrigin(capabilityOrigin(credentials.baseUrl), options.trustedLegacyOrigin)) return null;
  const legacySummary = options.legacySummary || await fetchLegacySummary(credentials, options);
  const summary = legacySummary && legacySummary.image_mcp;
  if (!isObject(summary) || summary.enabled !== true || summary.default_model !== 'gpt-image-2' ||
    !sameStrings(uniqueStrings(summary.tools), REQUIRED_TOOLS) || !positiveInteger(summary.max_concurrent_jobs) ||
    nonNegativeInteger(summary.max_queued_jobs) === null) return null;
  const payload = legacyPayload(summary);
  return { ...normalizeImageCapabilities(payload), legacy: true };
}

async function fetchLegacySummary(credentials, options) {
  try {
    const response = await options.fetch(`${String(credentials.baseUrl).replace(/\/+$/, '')}/gpteam/config-capabilities`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${credentials.apiKey}`,
        Accept: 'application/json',
    ...
    if (!response || !response.ok || response.status >= 300 && response.status < 400) return null;
...
function legacyPayload(summary) {
  const localParameters = [
    legacyParameter('format', 'string', { default: 'png', enum: ['png', 'jpeg', 'webp'], ownership: 'mcp_local', effect: 'gpteam_effective' }),
    legacyParameter('idempotency_key', 'string', { ownership: 'mcp_local', effect: 'gpteam_effective' }),
    legacyParameter('output_path', 'string', { ownership: 'mcp_local', effect: 'gpteam_effective' }),
    legacyParameter('overwrite', 'boolean', { default: false, ownership: 'mcp_local', effect: 'gpteam_effective' })
  ];
  const common = [
    legacyParameter('prompt', 'string', { required: true }),
    legacyParameter('size', 'string', { default: '1024x1024' }),
    legacyParameter('quality', 'string', { default: 'high', enum: ['low', 'medium', 'high', 'auto'] }),
    ...localParameters
  ];
  const profile = (parameters) => ({
    id: 'legacy-gpt-image-2', default_priority: 1, provenance: 'verified_builtin', eligible: true, parameters
  });
  return {
    object: 'gpteam.image_capabilities', schema_version: 1,
    revision: nonEmptyString(summary.revision) || 'legacy-gpt-image-2',
    observed_at: new Date().toISOString(), api_key: { id: 1 },
    group: { id: 1, name: 'legacy', platform: 'openai' },
    image_mcp: {
      enabled: true, blocking_reasons: [], default_models: { generate: 'gpt-image-2', edit: 'gpt-image-2' },
      tools: REQUIRED_TOOLS, max_concurrent_jobs: summary.max_concurrent_jobs,
      max_queued_jobs: summary.max_queued_jobs,
      models: [{
        id: 'gpt-image-2', platform: 'openai', contract_revision: 1, eligible: true,
        routable_by_contract: true, actions: {
          generate: { endpoint: '/v1/images/generations', eligible: true, execution_profiles: [profile(common)] },
          edit: { endpoint: '/v1/images/edits', eligible: true, execution_profiles: [profile([
            ...common,
            legacyParameter('images', 'string_array', { required: true, minimum_items: 1 }),
            legacyParameter('mask', 'string')
          ])] }
        }
      }]
    }
...
function legacyParameter(name, type, overrides = {}) {
  return {
    name, type, description: `${name} legacy compatibility parameter`, required: false,
    ownership: 'forwarded', effect: 'upstream_effective', ...overrides
  };
}
...
function sameOrigin(left, right) {
  try {
    return new URL(String(left)).origin.toLowerCase() === new URL(String(right)).origin.toLowerCase();
  } catch {
    return false;
  }
}
