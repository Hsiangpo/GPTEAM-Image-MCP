...
async function resolveImageCapabilities(deps, forceRefresh = false) {
  ...
  return fetchImageCapabilities(credentials, {
    ...
    forceRefresh,
    allowLegacyFallback: legacyFallbackEnabled(deps.env),
    trustedLegacyOrigin: deps.trustedLegacyOrigin || 'https://api.gpteamservices.com',
    legacySummary: deps.legacySummary
  });
}
...
function legacyFallbackEnabled(env = process.env) {
  return String(env && env.GPTEAM_IMAGE_MCP_ALLOW_LEGACY_FALLBACK || '').trim().toLowerCase() === 'true';
}
