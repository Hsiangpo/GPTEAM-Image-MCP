...
async function fetchImageOnce(fetchImpl, credentials, payload, options) {
  ...
  const endpoint = imageEndpointFromRequest(payload, options.action);
  try {
...
function imageEndpointFromRequest(payload, action) {
  if (action === 'generate') return '/images/generations';
  if (action === 'edit') return '/images/edits';
  return imageEndpointFromPayload(payload);
}
