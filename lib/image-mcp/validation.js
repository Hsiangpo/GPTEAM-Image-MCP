...
export function normalizeCapabilityRequest(input = {}, capabilities, options = {}) {
  ...
  for (const profile of actionContract.profiles || []) {
    try {
      candidates.push(normalizeForProfile(normalizedInput, profile, options));
    } catch (error) {
      failures.push(error);
    }
  }
...
function normalizeForProfile(input, profile, options) {
  ...
  for (const [name, value] of Object.entries(input || {})) {
    ...
    const normalizedValue = validateParameterValue(parameter, value);
    normalized[name] = normalizedValue;
    (parameter.ownership === 'mcp_local' ? local : forwarded)[name] = normalizedValue;
  }
  ...
  if (normalized.format && normalized.output_format && normalized.format !== normalized.output_format) {
    throw validationError('unsupported_parameter', 'format 和 output_format 不能互相冲突。', 'output_format', normalized.output_format);
  }
...
function validateParameterValue(parameter, value) {
  ...
  if (parameter.enum && parameter.enum.length && !parameter.enum.includes(value)) return invalidParameter(parameter, value);
